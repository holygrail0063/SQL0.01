      ...
      <section className="flex min-w-0 flex-col">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-line bg-panel px-6 py-5">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-slate-950">Sandbox</h1>
            <p className="mt-2 text-sm text-slate-600">Run read-only SQL against SQLBankTraining without correctness scoring.</p>
          </div>
          <Link className="rounded border border-line px-4 py-2 text-sm font-semibold text-slate-700 hover:border-brand-strong/50" href="/practice">Back to Practice</Link>
        </header>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-elevated px-6 py-3">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="inline-flex h-9 items-center gap-2 rounded border border-line px-3 text-sm text-slate-700 hover:border-brand-strong/50" onClick={() => setQuery("")} type="button">
              <RotateCcw size={15} /> Reset
            </button>
            <button className="inline-flex h-9 items-center gap-2 rounded bg-brand px-4 text-sm font-semibold text-slate-950 disabled:bg-slate-300" disabled={running} onClick={runQuery} type="button">
