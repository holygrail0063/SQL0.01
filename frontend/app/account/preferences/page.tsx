...
function LearningPreferencesContent() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10">
      ...
      <p className="mt-2 text-sm text-slate-400">Choose the SQL path and pace QueryRight should use when recommending lessons and practice.</p>
      <section className="mt-8 space-y-5 rounded border border-line bg-panel p-6">
        <label className="block text-sm text-slate-300">
          Learning Goal
          <select className="mt-2 h-11 w-full rounded border border-line bg-elevated px-3 text-slate-50 outline-none focus:border-brand" onChange={(event) => setRole(event.target.value)} value={role}>
            {activeRoleOptions.map((option) => <option key={option}>{option}</option>)}
          </select>
        </label>
        <label className="block text-sm text-slate-300">
          SQL Experience
          <select className="mt-2 h-11 w-full rounded border border-line bg-elevated px-3 text-slate-50 outline-none focus:border-brand" onChange={(event) => setLevel(event.target.value)} value={level}>
        ...
        {selectedCourse ? (
          <div className="rounded border border-line bg-elevated p-5">
            ...
            <h2 className="mt-2 text-xl font-semibold text-slate-50">{selectedCourse.shortTitle}</h2>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {selectedCourse.skills.slice(0, 10).map((skill) => (
                <span className="rounded border border-line bg-elevated px-2 py-1 text-xs text-slate-300" key={skill}>{skill}</span>
              ))}
            </div>
          </div>
        ) : (
          <div className="rounded border border-line bg-elevated p-5">
            ...
            <h2 className="mt-2 text-xl font-semibold text-slate-50">{role}</h2>
      ...
      {confirmingPathChange && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-5">
          <section className="w-full max-w-md rounded border border-line bg-panel p-6 shadow-2xl shadow-black/40">
            <h2 className="text-xl font-semibold text-slate-50">Update learning path?</h2>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button className="rounded border border-line px-4 py-2 text-sm text-slate-300 hover:border-brand-strong/50" onClick={() => setConfirmingPathChange(false)} type="button">Cancel</button>
