...
export function SchemaExplorer({ schema }: { schema: SchemaTable[] }) {
  ...
  return (
    <aside className="h-full border-r border-line bg-[#0a1322] p-4">
      <div className="mb-5 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-slate-300">
        <Database size={16} />
        Database
      </div>
      <div className="mb-4 rounded border border-line bg-panel px-3 py-2 text-sm text-mint">SQLBankTraining</div>
      <div className="space-y-1">
        {schema.map((table) => {
          ...
          return (
            <div key={table.table}>
              <button
                className="flex w-full items-center gap-1 rounded px-2 py-1.5 text-left text-sm text-slate-100 hover:bg-[#172238]"
