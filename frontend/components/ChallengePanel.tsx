  ...
  return (
    <section className="border-b border-line bg-panel px-6 py-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wider text-slate-500">SQLBank • Question #{String(challenge.id).padStart(3, "0")} • Progress {current} / {total}</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="space-y-4 rounded-lg border border-line bg-elevated p-4">
          <div>
            ...
            <pre className="mt-2 overflow-auto whitespace-pre-wrap rounded bg-editor p-3 font-mono text-xs leading-5 text-slate-300">{challenge.example_sql}</pre>
          </div>
          <div>
